...
def is_chinese(word):
    if word >= u'/u4e00' and word <= u'/u9fa5':
        return True
    else:
        return False
...
def drop_stopwords(contents):
    # 初始化获取停用词表
    stop = open('C:/Users/admin/Desktop/BERT_Chinese_Classification/dealText/data/word_deal/stop_word_cn.txt',
                encoding='utf-8')
    stop_me = open('C:/Users/admin/Desktop/BERT_Chinese_Classification/dealText/data/word_deal/stop_one_mx.txt',
                   encoding='utf-8')
    key_words = open('C:/Users/admin/Desktop/BERT_Chinese_Classification/dealText/data/word_deal/key_words.txt',
                     encoding='utf-8')
    # 分割停用词/自定义停用词/关键词
    stop_words = stop.read().split("/n")
    stop_me_words = stop_me.read().split("/n")
    key_words = key_words.read().split("/n")
    # 定义返回后的结果
    contents_new = []
    # 遍历处理数据
...
def deal_content(content):
    contents = [content]
    # 去除空格
    contents = remove_blank_space(contents)
    # 获取分词结果

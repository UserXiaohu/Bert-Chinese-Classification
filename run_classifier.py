...
class DreamProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        for index, test in enumerate(zip(x_train_df.values, y_train_df.values)):
            ...
            # text_a = tokenization.convert_to_unicode(str(test[0][1]))  # 要分类的文本
    ...
    def get_dev_examples(self, data_dir):
        ...
        x_test_df = pd.read_csv(x_test_path, encoding='utf-8')
        y_test_df = pd.read_csv(y_test_path, encoding='utf-8')
        test_data = []
        for index, test in enumerate(zip(x_test_df.values, y_test_df.values)):
            guid = 'train-%d' % index  # 参数guid是用来区分每个example的
            text_a = tokenization.convert_to_unicode(dt.deal_content(str(test[0][1])))  # 要分类的文本
            # text_a = tokenization.convert_to_unicode(str(test[0][1]))  # 要分类的文本
            label = str(test[1][1])  # 文本对应的情感类别
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    # --- save label2id.pkl ---
    # 在这里输出label2id.pkl , add by hyf 2020/5/26
